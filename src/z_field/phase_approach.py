...
def pol_function_diffable(apply_fn: Callable, params: Any, batch: Any, alpha: int):
...
def p_real(apply_fn: Callable, params: Any, batch: Any, alpha: int):
    """Calculate the real part of a complex phase of the sum of scalar properties.

    Args:
        apply_fn: Model function.
        params: Model parameters.
        batch: Batch object.
    ...
    """
    return pol_function_diffable(apply_fn, params, batch, alpha).real


def p_imag(apply_fn: Callable, params: Any, batch: Any, alpha: int):
    ...
    """
    return pol_function_diffable(apply_fn, params, batch, alpha).imag


def z_i_alpha_beta(apply_fn: Callable, params: Any, batch: Any):
    ...
    # Create gradient functions with apply_fn captured
    def p_real_wrapper(params, batch, alpha):
        return p_real(apply_fn, params, batch, alpha)

    def p_imag_wrapper(params, batch, alpha):
        return p_imag(apply_fn, params, batch, alpha)
